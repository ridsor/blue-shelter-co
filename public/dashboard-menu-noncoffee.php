...
require 'koneksi.php';
require 'crud.php';
$sql = "SELECT menu.foto, menu.id, menu.harga, menu.nm_menu, menu.kategori_id FROM menu INNER JOIN kategori ON menu.kategori_id=kategori.id WHERE kategori.slug='non-coffee' ORDER BY menu.created_at DESC";
...
<html lang="en">
  <head>
    ...
    <title>Dashboard Menu Coffee</title>
      ...
      <section class="px-4 lg:px-0">
        <div class="container max-w-7xl">
          <div
                    ...
                    <div class="p-7">
                      <form
                        ...
                        <div class="form-input w-full relative mb-6">
                          <select
                            ...
                            <?php while($row = mysqli_fetch_assoc($queryKategori)): ?>
                                ...
                                <option value="<?= $row['id'] ?>" data-kategori="non-coffee">
                                  <?= $row['nm_kategori'] ?>
                                </option>
                                <?php else: ?>
                                  <option value="<?= $row['id'] ?>">
                        ...
                        <div class="form-input mb-5">
                          ...
                          <label
                            for="foto"
                            class="block hover:ring hover:ring-slate-700"
                            tabindex="0">
                            <div
            ...
            <div
              id="list-menu"
              class="noncoffee row grid gap-5 gap-y-10 md:gap-10 [grid-template-columns:repeat(auto-fill,minmax(100px,1fr))] sm:[grid-template-columns:repeat(auto-fill,minmax(150px,1fr))] pb-5">
              <?php while($row = mysqli_fetch_assoc($query)): ?>
              <div>
                <div
                  class="relative menu-img border border-black rounded-[27px] bg-[rgba(217,217,217,.66)] [box-shadow:0_0_19px_0_rgba(0,0,0,.40)] h-[250px] object-cover overflow-hidden">
                  <img
                ...
                <div class="price text-lg font-light text-center">
                  Rp<?=  $row['harga'] ?>
                </div>
              </div>
              <?php endwhile; ?>
              <div
                class="bg-[rgba(217,217,217,0.66)] border border-black rounded-[27px] [box-shadow:0_0_19px_0_rgba(0,0,0,0.40)] h-[250px] flex justify-center items-center">
                <span
